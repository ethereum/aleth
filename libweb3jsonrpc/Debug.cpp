...
StandardTrace::DebugOptions debugOptions(Json::Value const& _json)
{
	...
	if (!_json["disableStorage"].empty())
		op.disableStorage = boost::lexical_cast<bool>(_json["disableStorage"].asString());
	if (!_json["disableMemory"].empty())
		op.disableMemory = boost::lexical_cast<bool>(_json["disableMemory"].asString());
	if (!_json["disableStack"].empty())
		op.disableStack = boost::lexical_cast<bool>(_json["disableStack"].asString());
	if (!_json["fullStorage"].empty())
		op.fullStorage = boost::lexical_cast<bool>(_json["fullStorage"].asString());
	return op;
}
...
Json::Value Debug::debug_traceTransaction(string const& _blockHash, int _txIndex, Json::Value const& _json)
{
	Json::Value ret;
	if (_txIndex < 0)
...
Json::Value Debug::debug_storageAt(string const& _blockHash, int _txIndex, string const& _address)
{
