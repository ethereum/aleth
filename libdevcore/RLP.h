...
public:
    ...
    /// Converts to RLPs collection object. Useful if you need random access to
    /// sub items or will iterate over multiple times.
...
private:
    ...
    /// @returns the size in bytes of the payload, as given by the RLP as
    /// opposed to as inferred from m_data.
    size_t length() const;
    ...
    /// @returns the size encoded into the RLP in @a _data and throws if _data
    /// is too short.
