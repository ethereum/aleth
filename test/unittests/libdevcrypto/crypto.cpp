...
BOOST_AUTO_TEST_CASE(PerfSHA256_32, *utf::disabled() *utf::label("perf"))
{
	...
	{
		clog << "Skipping test Crypto/devcrypto/PerfSHA256_32. Use --all or --performance to run it.\n";
		return;
	}
...
BOOST_AUTO_TEST_CASE(PerfSHA256_4000, *utf::disabled() *utf::label("perf"))
{
	if (!test::Options::get().performance)
	{
		clog << "Skipping test Crypto/devcrypto/PerfSHA256_4000. Use --all or --performance to run it.\n";
		return;
	}
