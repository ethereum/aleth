...
BOOST_AUTO_TEST_CASE(test_secrets_cpp_vectors)
{
	...
	ETH_CHECK(crypto::ecdh::agree(initR.secret(), recvR.pub(), ephemeralShared));
	Secret expected(fromHex("20d82c1092f351dc217bd66fa183e801234af14ead40423b6ee25112201c6e5a"));
	ETH_REQUIRE(expected == ephemeralShared);
	...
	{
		ETH_REQUIRE(ephemeralShared == *(Secret*)keyMaterialBytes.data());
	...
	ETH_REQUIRE(sharedSecret == fromHex("b65319ce56e00f3be75c4d0da92b5957d5583ca25eeeedac8e29b6dfc8b1ddf7"));
	...
	// test that keyMaterial = ecdhe-shared-secret || shared-secret
	{
		...
		bytes test(32);
		outRef.copyTo(&test);
		ETH_REQUIRE(expected == test);
	}
	
	// token: sha3(outRef)
	bytes token(32);
	sha3(outRef, bytesRef(&token));
	ETH_REQUIRE(token == fromHex("db41fe0180f372983cf19fca7ee890f7fb5481079d44683d2c027be9e71bbca2"));
	
	// aes-secret = sha3(ecdhe-shared-secret || shared-secret)
	sha3(keyMaterial, outRef); // output aes-secret
	bytes aesSecret(32);
	outRef.copyTo(&aesSecret);
	ETH_REQUIRE(aesSecret == fromHex("12347b4784bcb4e74b84637940482852fe25d78e328cf5c6f7a396bf96cc20bb"));
	...
	ETH_REQUIRE(macSecret == fromHex("2ec149072353d54437422837c886b0538a9206e6c559f6b4a55f65a866867723"));
	...
	
	{
		...
		ETH_REQUIRE(digest == fromHex("23e5e8efb6e3765ecae1fca9160b18df"));
	}
	...
	
	{
		...
		bytes digest(16);
		h.TruncatedFinal(digest.data(), 16);
		ETH_REQUIRE(digest == fromHex("ceed64135852064cbdde86e7ea05e8f5"));
	}
}
...
BOOST_AUTO_TEST_CASE(test_secrets_from_go)
{
	...
	ETH_REQUIRE(s_secp256k1->decryptECIES(recv.secret(), authPlain));
	bytes ackPlain = ackCipher;
	ETH_REQUIRE(s_secp256k1->decryptECIES(init.secret(), ackPlain));
	...
	bytes token(32);
	sha3(outRef, bytesRef(&token));
	ETH_REQUIRE(token == fromHex("0x3f9ec2592d1554852b1f54d228f042ed0a9310ea86d038dc2b401ba8cd7fdac4"));
	
	// aes-secret = sha3(ecdhe-shared-secret || shared-secret)
	sha3(keyMaterial, outRef); // output aes-secret
	bytes aesSecret(32);
	outRef.copyTo(&aesSecret);
	ETH_REQUIRE(aesSecret == fromHex("0xc0458fa97a5230830e05f4f20b7c755c1d4e54b1ce5cf43260bb191eef4e418d"));
	m_frameEnc.SetKeyWithIV(outRef.data(), h128::size, h128().data());
	m_frameDec.SetKeyWithIV(outRef.data(), h128::size, h128().data());
	...
	bytes macSecret(32);
	outRef.copyTo(&macSecret);
	ETH_REQUIRE(macSecret == fromHex("0x48c938884d5067a1598272fcddaa4b833cd5e7d92e8228c0ecdfabbe68aef7f1"));
	...
	
	{
		...
		bytes digest(32);
		h.Final(digest.data());
		ETH_REQUIRE(digest == fromHex("0x09771e93b1a6109e97074cbe2d2b0cf3d3878efafe68f53c41bb60c0ec49097e"));
	}
	
	// recover mac-secret by re-xoring remoteNonce
	bytes recoverMacSecretTest(32);
	(*(h256*)keyMaterial.data() ^ remoteNonce).ref().copyTo(&recoverMacSecretTest);
	ETH_REQUIRE(recoverMacSecretTest == macSecret);
...

	{
		...
		bytes digest(32);
		h.Final(digest.data());
		ETH_CHECK(digest == fromHex("0x75823d96e23136c89666ee025fb21a432be906512b3dd4a3049e898adb433847"));
	}
	...
	/// test macs of frame headers
	{
		...
		ETH_REQUIRE(*(h128*)encDigest.data() == *(h128*)provided.data());
	}
...
BOOST_AUTO_TEST_CASE(ecies_interop_test_primitives)
{
	...
	ETH_REQUIRE(emptyExpected == emptyTestOut);
	...
	ETH_REQUIRE(hash1Out == hash1Expected);
	...
	ETH_REQUIRE(hmacExpected == hmacOut);
	...
	ETH_REQUIRE(mac.asBytes() == tagExpected);
	...
	ETH_REQUIRE(kdfTest2 == kdfExpect2);
	...
	Secret agreeTest;
	ETH_CHECK(crypto::ecdh::agree(k.secret(), p, agreeTest));
	ETH_REQUIRE(agreeExpected == agreeTest);
	...
	ETH_REQUIRE(s_secp256k1->decryptECIES(kmK.secret(), kmPlain));
	ETH_REQUIRE(kmExpected == kmPlain);
	...
	ETH_REQUIRE(s_secp256k1->decryptECIES(kenc.secret(), plainTest1));
	ETH_REQUIRE(plainTest1 == expectedPlain1);
	...
	ETH_REQUIRE(s_secp256k1->decryptECIES(kenc.secret(), plainTest2));
	ETH_REQUIRE(plainTest2 == expectedPlain2);
	...
	ETH_REQUIRE(s_secp256k1->decryptECIES(kenc.secret(), plainTest3));
	ETH_REQUIRE(plainTest3 == expectedPlain3);
}
...
BOOST_AUTO_TEST_CASE(segmentedPacketFlush)
{
	...
	ETH_REQUIRE_EQUAL(61, payload.size());
	
	auto drains = (payload.size() + 3) / dequeLen;
	ETH_REQUIRE_EQUAL(4, drains);
	...
	for (unsigned i = 1; i < drains; i++)
	{
		...
		ETH_REQUIRE_EQUAL(0, n);
		ETH_REQUIRE_EQUAL(encframes.size(), i);
	}
	ETH_REQUIRE_EQUAL(1, w.mux(encoder, RLPXFrameWriter::MinFrameDequeLength, encframes));
	ETH_REQUIRE_EQUAL(encframes.size(), drains);
	ETH_REQUIRE_EQUAL(0, w.mux(encoder, RLPXFrameWriter::MinFrameDequeLength, encframes));
	ETH_REQUIRE_EQUAL(encframes.size(), drains);
	
	// we should now have a bunch of ciphertext in encframes
	ETH_REQUIRE(encframes.size() == drains);
	for (auto const& c: encframes)
	{
		ETH_REQUIRE_EQUAL(c.size(), RLPXFrameWriter::MinFrameDequeLength);
	}
	...
	ETH_REQUIRE_EQUAL(packets.size(), 1);
	ETH_REQUIRE_EQUAL(packets.front().size(), packetTypeRLP.size() + rlpPayload.out().size());
	ETH_REQUIRE_EQUAL(sha3(RLP(packets.front().data()).payload()), sha3(payload));
	ETH_REQUIRE_EQUAL(sha3(packets.front().type()), sha3(packetTypeRLP));
}
...
BOOST_AUTO_TEST_CASE(coalescedPacketsPadded)
{
	...
	deque<bytes> encframes;
	ETH_REQUIRE_EQUAL(4, w.mux(encoder, dequeLen, encframes));
	ETH_REQUIRE_EQUAL(0, w.mux(encoder, dequeLen, encframes));
	ETH_REQUIRE_EQUAL(1, encframes.size());
	...
	ETH_REQUIRE_EQUAL(expectedFrameSize, encframes[0].size());
	...
	bytesRef frame = frameWithHeader.cropped(h256::size);
	RLPXFrameInfo f(header);
	ETH_REQUIRE_EQUAL(f.multiFrame, false);
	for (RLPXPacket& p: r.demux(decoder, f, frame))
		packets.push_back(move(p));
	
	RLPStream rlpPayload;
	rlpPayload << stuff;
	ETH_REQUIRE_EQUAL(packets.size(), 4);
	while (!packets.empty())
	{
		ETH_REQUIRE_EQUAL(packets.back().size(), packetTypeRLP.size() + rlpPayload.out().size());
		ETH_REQUIRE_EQUAL(sha3(RLP(packets.back().data()).payload()), sha3(stuff));
		ETH_REQUIRE_EQUAL(sha3(packets.back().type()), sha3(packetTypeRLP));
		packets.pop_back();
	}
}
...
BOOST_AUTO_TEST_CASE(singleFramePacketFlush)
{
	...
	ETH_REQUIRE_EQUAL(1, w.mux(encoder, dequeLen, encframes));
	ETH_REQUIRE_EQUAL(0, w.mux(encoder, dequeLen, encframes));
	ETH_REQUIRE_EQUAL(1, encframes.size());
	ETH_REQUIRE_EQUAL(dequeLen, encframes[0].size());
...
BOOST_AUTO_TEST_CASE(multiProtocol)
{
	...
	deque<bytes> encframes;
	ETH_REQUIRE_EQUAL(2, mw[0]->mux(encoder, dequeLen, encframes));
	ETH_REQUIRE_EQUAL(2, mw[1]->mux(encoder, dequeLen, encframes));
	ETH_REQUIRE_EQUAL(0, mw[0]->mux(encoder, dequeLen, encframes));
	ETH_REQUIRE_EQUAL(0, mw[1]->mux(encoder, dequeLen, encframes));
	ETH_REQUIRE_EQUAL(2, encframes.size());
	...
	for (size_t i = 0; i < encframes.size(); i++)
	{
		...
		{
			ETH_REQUIRE_EQUAL(f.protocolId, p.cap());
			packets.push_back(move(p));
		}
	}
	...
	for (size_t i = 0; i < packets.size(); i++)
	{
		...
		ETH_REQUIRE_EQUAL(packets[i].size(), packetTypeRLP.size() + vRlpPayloads[prot].out().size());
		ETH_REQUIRE_EQUAL(sha3(RLP(packets[i].data()).payload()), sha3(vStuff[prot]));
		ETH_REQUIRE_EQUAL(sha3(packets[i].type()), sha3(packetTypeRLP));
	}
}
...
BOOST_AUTO_TEST_CASE(oddSizedMessages)
{
	...
	ETH_REQUIRE_EQUAL(n, 1);
	n = w.mux(encoder, dequeLen, encframes);
	ETH_REQUIRE_EQUAL(n, 1);
	ETH_REQUIRE_EQUAL(encframes.size(), 3);
	...
	ETH_REQUIRE_EQUAL(packets.size(), totalMessages);
	ETH_REQUIRE_EQUAL(sha3(RLP(packets.front().data()).payload()), sha3(packetsOut[0]));
	ETH_REQUIRE_EQUAL(sha3(RLP(packets.back().data()).payload()), sha3(packetsOut[1]));
	ETH_REQUIRE_EQUAL(sha3(packets.front().type()), sha3(packetTypeRLP));
	ETH_REQUIRE_EQUAL(sha3(packets.back().type()), sha3(packetTypeRLP));
}
...
BOOST_AUTO_TEST_CASE(pseudorandom)
{
	...
	ETH_REQUIRE_EQUAL(numMessages, packetsSent.size());
	...
	ETH_REQUIRE_EQUAL(numMessages, packetsReceived.size());

	for (size_t i = 0; i < numMessages; i++)
	{
		ETH_REQUIRE(packetsReceived[i].type() == packetTypeRLP);
		ETH_REQUIRE_EQUAL(sha3(RLP(packetsReceived[i].data()).payload()), sha3(packetsSent[i]));
	}
}
...
BOOST_AUTO_TEST_CASE(randomizedMultiProtocol)
{
	...
	ETH_REQUIRE_EQUAL(numMessages, total);
	...
	for (size_t i = 0; i < numSubprotocols; ++i)
		ETH_REQUIRE_EQUAL(msgPerSubprotocolReceived[i], msgPerSubprotocolSent[i]);

	for (size_t i = 0; i < numMessages; i++)
	{
		ETH_REQUIRE(packetsReceived[i].type() == packetTypeRLP);
		ETH_REQUIRE_EQUAL(sha3(RLP(packetsReceived[i].data()).payload()), sha3(packetsSentShuffled[i]));
	}
}
