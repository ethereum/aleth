...
public:
	...
	template <class T> static bigint txGas(T const& _data, u256 _gas = 0, TransactionType _type = TransactionType::MessageCall) { bigint ret = c_txGas + _gas; if (_type == TransactionType::ContractCreation) ret += c_createGas;  for (auto i: _data) ret += i ? c_txDataNonZeroGas : c_txDataZeroGas; return ret; }
