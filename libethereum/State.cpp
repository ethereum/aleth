...
void State::dumpToFile()
{

    BlockInfo bi = m_previousBlock;
    map<Address, u256> fullAddressMap = this -> addresses();
         ...
         ++it )
    {
        ...
        string  balance = escaped(toBigEndianString(this -> balance(key)));
        string  nonce   = escaped( toString( this -> transactionsFrom(key) ) );

        map<u256, u256> storage = this -> storage(key);
        storage.empty();
        ...
        if (!storage.empty())
        {
            storage_key = toHex( toBigEndianString( this -> storageRoot(key) ) );
        ...
        string code;
        if (!this -> code(key).empty())
        {
            code = "0x" + toHex( this ->  code(key) );
        } else
        {
            code = "0x";
        }
        ...
        this -> storage(key);
        os << "    \"" << key_s << "\": {\n";
        os << "      \"balance\": \""      << this -> balance(key)          << "\",\n";
        os << "      \"code\": \""         << code    << "\",\n";
        os << "      \"nonce\": \""        << this -> transactionsFrom(key) << "\",\n";
        os << os_storage.str();
