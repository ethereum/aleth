	...
	while (true)
	{
		...
		else if (c && cmd == "transact")
		{
			...
			else
			{
				...
				u256 minGas = (u256)Transaction::gasRequired(data, 0, fields[0].length() == 0 ? TransactionType::ContractCreation : TransactionType::MessageCall);
				if (size < 40)
				{
					if (size > 0)
		...
		else if (c && cmd == "send")
		{
			...
			else
			{
				...
				else
				{
					...
					auto blockData = bc.block(h);
					BlockInfo info(blockData);
					u256 minGas = (u256)Transaction::gasRequired(bytes(), 0, fields[0].length() == 0 ? TransactionType::ContractCreation : TransactionType::MessageCall);
					try
					{
						Address dest = h160(fromHex(fields[0], WhenError::Throw));
