...
uint64_t VM::decodeJumpDest(const byte* const _code, uint64_t& _pc)
{
	...
	uint64_t dest      = to_integer(_code[_pc++]);
	dest = (dest << 8) | to_integer(_code[_pc++]);
	return dest;
}
...
uint64_t VM::decodeJumpvDest(const byte* const _code, uint64_t& _pc, byte _voff)
{
	...
	if (_voff >= n) _voff = (byte)(to_integer(n) - 1);  // if offset overflows use default jump
	pc += to_integer(_voff) * 2;                // adjust inout pc before index destination in table
...
void VM::interpretCases()
{
	...
	{	
		...
		CASE(PUSHC)
		{
			...
			unsigned off;
			++m_PC;
			off = to_integer(m_code[m_PC++]) << 8;
			off |= to_integer(m_code[m_PC++]);
			m_PC += to_integer(m_code[m_PC]);
			m_SPP[0] = m_pool[off];
		...
		CASE(PUSH1)
		{
			...
			updateIOGas();
			++m_PC;
			m_SPP[0] = to_integer(m_code[m_PC]);
			++m_PC;
		}
		CONTINUE
		...
		CASE(PUSH32)
		{
			...
			for (++m_PC; numBytes--; ++m_PC)
				m_SPP[0] = (m_SPP[0] << 8) | to_integer(m_code[m_PC]);
		}
		CONTINUE
