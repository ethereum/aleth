...
void VM::interpretCases()
{
	...
	union StackItem {
		...
		StackItem() { clear(); }
		u256*   cheat() { return (u256*)wdata; }
		u256&    wide() { return *cheat(); }
		uint8_t  wdata [sizeof(u256)];
		uint64_t v64x4 [4];		
		uint32_t v32x8 [8];
		uint16_t v16x16[16];
		uint8_t  v8x32 [32];
	...
	//
	// input bytes are the inline simd type descriptors for the input vectors on the stack
	//
	void xadd    (uint8_t);
	void xmul    (uint8_t);
	void xsub    (uint8_t);
	...
	void xput    (uint8_t);
	void xget    (uint8_t);
	void xswizzle(uint8_t, uint8_t);
	void xshuffle(uint8_t, uint8_t, uint8_t);
};
