...
namespace host
{
class host : public evmc_context  // FIXME: Make private?
{
protected:
    explicit host(const evmc_host_interface* vtable) noexcept : evmc_context{vtable} {}
...
public:
    ...
    virtual evmc_bytes32 get_storage(
        const evmc_address& _addr, const evmc_bytes32& _key) noexcept = 0;

    virtual evmc_storage_status set_storage(evmc_address const& _addr, evmc_bytes32 const& _key,
        evmc_bytes32 const& _value) noexcept = 0;

    virtual evmc_uint256be get_balance(const evmc_address& _addr) noexcept = 0;

    virtual size_t get_code_size(const evmc_address& _addr) noexcept = 0;

    virtual evmc_bytes32 get_code_hash(const evmc_address& _addr) noexcept = 0;

    virtual size_t copy_code(evmc_address const& _addr, size_t _codeOffset, byte* _bufferData,
        size_t _bufferSize) noexcept = 0;

    virtual void selfdestruct(
        evmc_address const& _addr, evmc_address const& _beneficiary) noexcept = 0;

    virtual evmc_result call(evmc_message const& _msg) noexcept = 0;

    virtual evmc_tx_context get_tx_context() noexcept = 0;

    virtual evmc_bytes32 get_block_hash(int64_t _number) noexcept = 0;

    virtual void emit_log(evmc_address const& _addr, uint8_t const* _data, size_t _dataSize,
        evmc_uint256be const _topics[], size_t _numTopics) noexcept = 0;
};
}  // namespace host
}  // namespace evmc
...
public:
    ...
    bool account_exists(evmc_address const& _addr) noexcept override;

    evmc_bytes32 get_storage(evmc_address const& _addr, evmc_bytes32 const& _key) noexcept override;

    evmc_storage_status set_storage(evmc_address const& _addr, evmc_bytes32 const& _key,
        evmc_bytes32 const& _value) noexcept override;

    evmc_uint256be get_balance(evmc_address const& _addr) noexcept override;

    size_t get_code_size(evmc_address const& _addr) noexcept override;

    evmc_bytes32 get_code_hash(evmc_address const& _addr) noexcept override;

    size_t copy_code(evmc_address const& _addr, size_t _codeOffset, byte* _bufferData,
        size_t _bufferSize) noexcept override;

    void selfdestruct(evmc_address const& _addr, evmc_address const& _beneficiary) noexcept override;

    evmc_result call(evmc_message const& _msg) noexcept override;

    evmc_tx_context get_tx_context() noexcept override;

    evmc_bytes32 get_block_hash(int64_t _number) noexcept override;

    void emit_log(evmc_address const& _addr, uint8_t const* _data, size_t _dataSize,
        evmc_uint256be const _topics[], size_t _numTopics) noexcept override;
