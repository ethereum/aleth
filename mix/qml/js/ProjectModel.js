...
function checkRegistration(dappUrl, addr, callBack)
{
	...
	if (dappUrl.length > 0)
	{
		...
		requests.push({
			jsonrpc: "2.0",
			method: "eth_call",
			params: [ { "gas": 2000, "to": '0x' + addr, "data": data } ],
			id: jsonRpcRequestId++
		});
	...
	else
	{
		...
		requests.push({
						  ...
						  jsonrpc: "2.0",
						  method: "eth_call",
						  params: [ { "gas" : 2000, "to": '0x' + addr, "data": "0xec7b9200" + paramTitle.encodeValueAsString() } ],
						  id: jsonRpcRequestId++
					  });
		...
		rpcCall(requests, function (httpRequest, response) {
			...
			if (noOwner)
			{
				requests.push({
							  ...
							  jsonrpc: "2.0",
							  method: "eth_transact",
							  params: [ { "gas": 2000, "to": '0x' + addr, "data": "0x1c83171b" + paramTitle.encodeValueAsString() } ],
							  id: jsonRpcRequestId++
						  });
			}
			else
			...
			requests.push({
						  ...
						  jsonrpc: "2.0",
						  method: "eth_transact",
						  params: [ { "gas": 2000, "to": '0x' + addr, "data": "0x5d574e32" + paramTitle.encodeValueAsString() + deploymentDialog.packageHash } ],
						  id: jsonRpcRequestId++
					  });
			rpcCall(requests, function (httpRequest, response) {
				callBack();
...
function registerToUrlHint()
{
	...
	requests.push({
				  ...
				  jsonrpc: "2.0",
				  method: "eth_transact",
				  params: [ { "gas": 2000, "to": '0x' + deploymentDialog.urlHintContract, "data": "0x4983e19c" + deploymentDialog.packageHash + paramUrlHttp.encodeValueAsString() } ],
				  id: jsonRpcRequestId++
			  });
